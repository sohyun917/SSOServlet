...
public class MyInfoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String memberId = request.getParameter("member-id");	//아이디를 가져왔을 때 아이디가 있으면 페이지를 보여준다.
		if(memberId != null) {
			// id가 존재하면 DB에서 데이터를 가져옴
			MemberService mService = new MemberService();
			Member member = mService.printOneById(memberId);	//아이디를 받아서 하나의 행에대한 값을 전부 가져옴
			request.setAttribute("member", member);
			request.getRequestDispatcher("/WEB-INF/views/member/myInfo.jsp").forward(request, response);			
		}else {
			// id가 없을 때 에러페이지로 이동(setAttribute로 title과 메세지를 변경해서 서비스마다 다르게 설정가능)
